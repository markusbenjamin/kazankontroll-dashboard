...
def process_and_save_heatmeter_readings(daystamp = datetime.now().strftime("%Y-%m-%d"), minute_step = 5, time_offset = 0):
    ...
    for minute in range(0, 24*60, minute_step):
        line = [minute]
        for cycle in range(1,5):
            preceding, succeeding = find_closest_in_nested(heat_stock[cycle - 1],minute,0)
            val = 'n'
            if preceding != 'n':
                preceding_time = preceding[0]
...
if __name__ == "__main__":
    ...
    #data_to_plot = []
    #for cycle in range(1,5):
    #    data_to_plot.append(transpose([transpose(heat_power_interpolated)[0],transpose(heat_power_interpolated)[cycle]]))
