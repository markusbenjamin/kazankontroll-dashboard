...
def calculate_relative_frequency(numbers_list):
    ...
    for num in frequency_dict:
        frequency_dict[num] = frequency_dict[num] / total_numbers
...
def do_ocr_on_cycles(cycle_crops):
    ...
    for cycle in range(1,5):
        ...
        for char in range(1,5):
            ...
            cycle_char_probs[cycle - 1].append(char_probs)
...
def do_bayesian_inference_reading(inference_time):
    ...
    with open(f"{belief_file_path}\\heatmeter_belief_state.csv", 'rb') as file:
        ...
        print(f"Belief line found: {line}")
    ...
    print(f"Loaded priors: {timed_priors}")

    if True in data_newer_than_latest_prior:
        # Calculate elapsed heating time on each cycle
        elapsed_heating_times = [0,0,0,0]
        for cycle in range(1,5):
            ...
            while (load_day + timedelta(days = -1)).day != inference_time.day:
                print(f"load day: {load_day.day}, inference day: {inference_time.day}")
            ...
            for n in range(len(pump_state_changes)):
                if pump_state_changes[n][1] == 1:
                    ...
                    if 0 < heating_time:
                        elapsed_heating_times[cycle - 1] += heating_time
        ...
        cycle_char_probs = do_ocr_on_cycles(cycle_crops)

        # Inference parameters
        OCR_prob_cutoff = 0.025
        ...
        for cycle in range(1,5):
            if timed_priors[cycle - 1][0] < inference_time:
                print(f"Start inference for cycle {cycle}.")
                print(f"OCR input: {cycle_char_probs[cycle - 1]}.")
                chars_with_enough_prob_pre = []
                not_enough_OCR_input = False
                for prob_dict in cycle_char_probs[cycle - 1]:
                    filtered_digits = [digit for digit, prob in prob_dict.items() if prob >= OCR_prob_cutoff]
                    if not filtered_digits:
                        not_enough_OCR_input = True
                    chars_with_enough_prob_pre.append(filtered_digits)

                if not not_enough_OCR_input:
                    # Select chars that are probable enough for consideration
                    chars_with_enough_prob = []
                    for char_list in chars_with_enough_prob_pre:
                        if 8 in char_list or 9 in char_list:
                            new_list = list({0}.union(char_list))
                        else:
                            new_list = char_list
                        chars_with_enough_prob.append(new_list)

                    print(f"Probable chars: {chars_with_enough_prob}.")
                    ...
                    current_probable_states = [energy for energy, prob in timed_priors[cycle - 1][1].items() if prob > 0]
                    ...
                    for energy in energy_readouts_with_enough_evidence:
                        ...
                        for char_index, char in enumerate(state):
                            total_prob += cycle_char_probs[cycle - 1][char_index].get(char, 0)

                        # Average the probability and append to the list
                        average_prob = total_prob / 4
                        if 0 < average_prob:
                            ocr_likelihoods.append((energy, average_prob))
                    
                    # Calculate heating power based likelihood component
                    print(f"Current probable energy states: {current_probable_states}.")
                    print(f"Next energy state candidates based on OCR evidence: {ocr_likelihoods}.")
                    power_likelihoods = []

                    print(f"Elapsed heating time: {elapsed_heating_times[cycle - 1]} hours.")
                    for next_energy, _ in ocr_likelihoods:
                        ...
                        for previous_energy in current_probable_states:
                            ...
                            weight = power_weighting_function(cycle, previous_energy, next_energy, elapsed_heating_times[cycle - 1], weibull_parameters[cycle - 1])
                            total_likelihood += prob * weight
                        if 0 < total_likelihood:
                            power_likelihoods.append((next_energy, total_likelihood))
                    
                    print(f"Next energy state candidates based on heating: {power_likelihoods}.")
                    ...
                    if len(belief_update) > 0:
                        ...
                        timed_priors[cycle - 1][1] = {energy:prob for energy, prob in posterior.items() if prob > 0}
                        update_on_cycle[cycle - 1] = True
                    else:
                        print(f"Not enough evidence for belief update on cycle {cycle}.")
                else:
                    print(f"Not enough OCR input, exiting inference for cycle {cycle}.")
            else:
                print(f"Prior newer than inference time for {cycle}.")
        ...
        if True in update_on_cycle:
            ...
            with open(f"{belief_file_path}/heatmeter_belief_state.csv", 'a', newline='') as file:
                writer = csv.writer(file, delimiter=';')
                writer.writerow(belief_state_string.split(';'))
    else:
        print(f"All priors newer than inference time.")
...
if __name__ == "__main__":
    ...
    start_day = datetime(2023, 12, 5)
    end_day = datetime(2023, 12, 5)
    day = start_day
    while day <= end_day:
        ...
        for timestamp in timestamps:
            ...
            do_bayesian_inference_reading(inference_time)
