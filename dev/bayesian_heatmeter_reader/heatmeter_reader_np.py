    ...
    with Image.open(img_path) as img:
        ...
        for cycle in range(1, 5):
            try:
                ...
                print(f"Cycle {cycle} successfully cropped and saved.")
            except Exception as e:
...
def do_bayesian_inference_reading(inference_time):
    ...
    with open(f"{belief_file_path}\\heatmeter_belief_state.csv", 'rb') as file:
        ...
        print(f"Belief line found: {line.strip()}")
    ...
    print(f"Loaded belief state:")
    for cycle in range(1,5):
        print(f"\tCycle {cycle}: {timed_priors[cycle - 1][0].strftime('%m-%d %H:%M')}, {timed_priors[cycle - 1][1]}")
    ...
    if True in data_newer_than_latest_prior:
        ...
        for cycle in range(1,5):
            if timed_priors[cycle - 1][0] < inference_time:
                ...
                cycle_char_probs = do_ocr_on_cycle(cycle, cycle_crop)
                for char in range(4):
                    print(f"\t\tChar {char+1} probs: {cycle_char_probs[char]}")
                ...
                if not not_enough_OCR_input:
                    ...
                    print(f"\tElapsed heating time: {round(elapsed_heating_time,2)} hours.")
                    ...
                    print(f"\tOf these with non-zero likelihood based on heating: {power_likelihoods}.")
    ...
    print(f"End inference for {inference_time.strftime('%m-%d %H:%M')}.\n")
...
if __name__ == "__main__":
    if True:
        archetype_vectors_2D = [] # lehet, hogy ez nem működik, ha tényleg nem, akkor RasPin lévő nem np verzióból kell kinézni
        ...
        start_day = datetime(2024, 2, 7)
        end_day = datetime(2024, 2, 7)
        day = start_day
        while day <= end_day:
        ...
        try:
            ...
            hour_minute_stamp = capture_time.strftime("%H:%M")
            image_filename = capture_image()
            crop_cycles_and_save(image_filename)
            ...
            do_bayesian_inference_reading(capture_time)
        except Exception as e:
